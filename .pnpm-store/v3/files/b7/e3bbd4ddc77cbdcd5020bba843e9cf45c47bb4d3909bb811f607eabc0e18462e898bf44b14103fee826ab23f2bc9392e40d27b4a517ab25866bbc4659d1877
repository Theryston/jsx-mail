...
function getPkgJsonByName(packages) {
...
async function createChangeset(changedPackages, allPackages) {
  ...
  if (allPackages.length > 1) {
    ...
    let pkgJsonsByName = getPkgJsonByName(allPackages), pkgsLeftToGetBumpTypeFor = new Set(packagesToRelease), pkgsThatShouldBeMajorBumped = (await askCheckboxPlus(bold(`Which packages should have a ${red("major")} bump?`), [ {
      name: "all packages",
...
function isListablePackage(config, packageJson) {
  return !config.ignore.includes(packageJson.name) && (packageJson.version || !packageJson.private);
}

async function add(cwd, {empty: empty, open: open}, config) {
  const packages = (await getPackages.getPackages(cwd)).packages.filter((pkg => isListablePackage(config, pkg.packageJson))), changesetBase = path__default.default.resolve(cwd, ".changeset");
  let newChangeset;
  if (empty) newChangeset = {
    ...
    newChangeset = await createChangeset(changedPackagesName, packages), printConfirmationMessage(newChangeset, packages.length > 1), 
