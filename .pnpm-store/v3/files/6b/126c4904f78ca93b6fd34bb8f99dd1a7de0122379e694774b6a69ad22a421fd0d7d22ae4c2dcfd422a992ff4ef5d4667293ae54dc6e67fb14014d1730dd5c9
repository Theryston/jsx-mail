...
async function updateChangelog(changelogPath, changelog, name, prettierConfig) {
  let templateString = `\n\n${changelog.trim()}\n`;
  try {
    fs__default.default.existsSync(changelogPath) ? await prependFile(changelogPath, templateString, name, prettierConfig) : await writeFormattedMarkdownFile(changelogPath, `# ${name}${templateString}`, prettierConfig);
  } catch (e) {
    console.warn(e);
  }
}
...
async function prependFile(filePath, data, name, prettierConfig) {
  ...
  if (!fileData) {
    ...
    return void await writeFormattedMarkdownFile(filePath, completelyNewChangelog, prettierConfig);
  }
  const newChangelog = fileData.replace("\n", data);
  await writeFormattedMarkdownFile(filePath, newChangelog, prettierConfig);
}

async function writeFormattedMarkdownFile(filePath, content, prettierConfig) {
  await fs__default.default.writeFile(filePath, prettier__default.default.format(content, _objectSpread2(_objectSpread2({}, prettierConfig), {}, {
    filepath: filePath,
    parser: "markdown"
  })));
}
