...
import { getChangedPackagesSinceRef, add as add$1, commit, getCurrentCommitId, tag, getAllTags } from '@changesets/git';
...
async function add(cwd, {
  ...
  const packages = (await getPackages(cwd)).packages.filter(pkg => isListablePackage(config, pkg.packageJson));
  const changesetBase = path.resolve(cwd, ".changeset");
  let newChangeset;
