...
async function updateChangelog(changelogPath, changelog, name, prettierConfig) {
  ...
  try {
    if (fs__default['default'].existsSync(changelogPath)) {
      ...
      await writeFormattedMarkdownFile(changelogPath, `# ${name}${templateString}`, prettierConfig);
    }
  } catch (e) {
    console.warn(e);
  }
...
async function prependFile(filePath, data, name, prettierConfig) {
  ...
  if (!fileData) {
    ...
    await writeFormattedMarkdownFile(filePath, completelyNewChangelog, prettierConfig);
    return;
  }
