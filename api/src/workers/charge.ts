...
async function chargeMessage() {
	...
	for (const { userId, _count: { id: messagesAmount } } of messages) {
		try {
...
async function chargeBandwidth() {
	...
	for (const { userId, _sum: { size } } of fileDownloads) {
...
async function chargeStorage() {
	...
	for (const { userId, _sum: { size } } of storageSize) {
...
async function removeBalance({ amount, userId, style, description }: {
	amount: number
	userId: string
