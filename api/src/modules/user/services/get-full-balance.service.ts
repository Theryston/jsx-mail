...
export class GetFullBalanceService {
	...
	async execute(userId: string) {
		...
		const { _sum: { amount: currentMonthAddedBalance } } = await this.prisma.transaction.aggregate({
			where: {
				userId,
				deletedAt: {
					isSet: false
		...
		const { _sum: { amount: currentMonthChargedBalance } } = await this.prisma.transaction.aggregate({
			where: {
				userId,
				deletedAt: {
					isSet: false
