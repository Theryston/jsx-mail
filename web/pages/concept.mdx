...
In Jsx Mail we have 2 sides. The first side is the JSX side, it should be a folder where inside it there is a file called `index.tsx` which is the [configuration file](/config-files/index-file). In this folder there is what we call the jsx side which is where all your email templates are, all your jsx code.
