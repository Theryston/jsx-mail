...
export default function MyFirst({ name, bio }) {
  return (
    <div>
      <img src={IconPng} alt="I'm an Icon" />
      <h1 style={Styles.Title}>Hello {name}</h1>
      <MyFirstComponent bio={bio} />
    </div>
  );
...
### Log Into JSX Mail Cloud

If you want to use the JSX Mail Cloud (to storage your images, send the emails and more...) you need to log into the JSX Mail Cloud. This step is not require and if you don't want to use JSX Mail Cloud just make the following changes in `jsx-mail.config.js` file:

```js copy {4,5} filename="jsx-mail.config.js"
...
<Callout emoji="🌟">
  ...
  refer to [this guide](/docs/cloud#production-login) for instructions on how to
  proceed.
</Callout>
